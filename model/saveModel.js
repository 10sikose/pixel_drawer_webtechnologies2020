...
export default class SaveModel {
    ...
    {
      if ( ! empty( $_POST ) ) {
      let image = prompt('Enter the image name:', '');
      // If no name was given, try again
      if (image === '')
      {
        saveClicked();
        return;
      }
      // If cancel was pressed, break operation
      if (image == null)
        return;
      // Save image
      ...
      let sentVars = 'name=' + image + '&data=' + canvasData;
      // Send request
      xmlRequest.send(sentVars);
    }
    }
